...
v0.2.0 落地后对照本文档逐项代码复核的结果。**主干功能全部上线，但以下承诺未兑现或只落地了一半**——原状态行"全部落地"的表述以此为修正：
...
| P0-3 MCP 笔记写工具 | ⚠️ 部分 | 写工具（create/update/delete + 文件夹管理）与护栏已落地；**FTS 检索升级未做**，`searchNotes` 仍为 LIKE 模糊匹配，外部 Agent 检索质量依旧低于应用内 |
...
| P1-5 摘录卡体系 | ⚠️ 部分 | 结构约定、两步 CoT 提炼入口、跨页多段累加、「我的想法」分离均已落地；**并排预览确认未做**（提炼结果直接入库/追加）；**多模态重识别未做**（提炼管线只接受文本输入）；**聚合管线（摘录卡→精读卡→概念页）未做** |
| P2-1 旧版 Agent 清理 | ✅ 完整 | 全链路移除，`agentLegacyMode` 设置键静默兼容 |
| P2-2 图谱/列表/体检 | ⚠️ 部分 | 笔记图谱视图与体检（孤立/断链/无标题/无标签/陈旧）已落地；**列表增强（多条件排序、批量打标签/移动）未做** |
...
> 📝 **复审注记（2026-09-26）**：写工具、文件夹管理与护栏已按本节落地，集成文档与测试同步齐全；唯独"检索升级为 FTS"未实施——MCP `search_notes` 仍是四字段 LIKE 模糊匹配，未读 `notes_fts` 虚表。已列入文末 P3-5。
...
1. **"全部落地"言过其实**：12 个路线图项中 4 项为部分落地，共 9 个子承诺未兑现（FTS 检索、页面类型数据模型、系统页维护机制、`.obsidian/` 配置、自定义模板、提炼并排预览、多模态重识别、聚合管线、列表增强）。状态行与各痛点节已据实修正。
...
4. **vault 同步纯手动**：无启动时同步、定时或文件 watcher——Obsidian 侧编辑要等用户主动按按钮才进库，双应用协作存在实际体验断点。
