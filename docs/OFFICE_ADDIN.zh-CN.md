...
- 在文末插入**参考文献表**：位置固定为一个域，正文增删引用后一键刷新重排。
- 切换引用样式：GB/T 7714-2015 顺序编码制、GB/T 7714-2015 著者-出版年、APA 7、IEEE。
- 单条引用的页码/前后缀/隐藏作者。
...
1. **连接**：PaperQuay → 设置 → 文献库 → 「Word 加载项（Office 桥）」→ 确认「运行中」→ 点「复制连接信息」（形如 `23120:3f9c…`）→ 粘进加载项顶部的连接框 → 「连接」。连接信息含本机端口与令牌，等价于本机密码，不要外传。
...
5. **插入参考文献表**：把光标放在文末（或已有表的位置）→「插入参考文献表」。表是一个域，标题默认为「参考文献」，可在加载项里改。
...
- **引用域**：每个引用是独立 ContentControl，`tag = pq:c|<citeId>`（短标签，规避 `ContentControl.tag` 的 64 字符上限）。域里的文本就是引用文本（如 `[1]` 或 `[1-3]`）。
...
- 手工回归：Word 桌面版插入 3 条 → 插入参考文献表 → 中间补一条 → 刷新 → 编号重排 → 关闭重开文档后仍可刷新。
