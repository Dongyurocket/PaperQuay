...
- **笔记语义检索（向量 + FTS5 + RRF 混合）**：复用阅读器 Embedding 配置与 RAG 基础设施，笔记在保存、导入、vault 回导后异步增量索引，软删除立即失效；内置 Agent 与外部 MCP 的笔记检索同时支持语义与关键词召回，返回 `retrievalMode` / `channels` 标明命中通道，未配置或服务异常时自动降级为关键词检索并给出提示。笔记工作区工具栏新增「重建笔记语义索引」入口，排队、完成、失败与最后错误可见。锚点、文献引用节点与原文快照属证据内容，不进入向量语料。
- **摘录提炼并排预览与多模态重识别**：提炼不再直接落库，先打开左右对照面板——左侧原始识别文本、右侧可编辑的标题与提炼正文，确认才写入，可随时取消；追加到已有摘录卡同样先预览，新锚点与新段落插入「💭 我的想法：」之前，已有锚点保持不变。预览内可用视觉模型重识别公式 / 表格区域（裁切原 PDF 区域图送入），模型不支持或调用、切片失败时自动回退纯文本；采用重识别结果的锚点标记 `aiEnhanced`，编辑器显示「AI 重识别」来源。
- **笔记聚合配方与系统页自动维护**：Agent 可按配方把摘录卡聚合为精读卡 / 概念页，要点逐条回链来源摘录卡，正文引用经 `[n]` 还原为真实文献引用节点；每次成功写入追加 `log` 记录日期、操作与涉及笔记，每累计 5 次成功写操作刷新 `index` / `overview` 导航页。系统页按内容标记判定归属，用户手改过的页面自动维护直接跳过，不覆盖手写内容。
- **体检报告接入 Agent 修复**：体检卡新增「让 Agent 修复」入口，一键打开 Agent 并预填修复指令；删除类问题默认只列清单，断链优先按已有 `noteId` 重挂，锚点与原文快照只可保真不可改写。
- **八种页面类型（page_kind）贯通全链路**：`notes.page_kind` 独立于来源语义 `type`，旧数据在打开数据库时幂等回填（`highlight → excerpt`、`ai-chat → qa`）；页面类型可在编辑器模板、笔记侧栏过滤、内置 Agent、MCP 写入 / 检索与 vault frontmatter 中传递，非法值直接被拒绝。
- **笔记 vault 自动同步**：已配置 vault 目录时，笔记工作区打开即静默同步一次；设置中提供默认关闭、15–60 分钟可调的定时同步（默认 30 分钟）。
- **笔记列表排序与批量整理**：列表支持按更新时间、创建时间、标题排序；选中多篇后可批量追加标签或移动文件夹，只组装 `tags` / `folderId` 补丁，正文、锚点与摘录快照不受影响。

### 优化

- **Markdown 往返保真**：新增共享 Markdown → Tiptap 解析器（`src/shared/markdownToTiptap.cjs`），vault 回导、MCP 笔记写入与编辑器兜底统一走该解析器，标题、段落、有序 / 无序列表、粗体、斜体、删除线、行内代码、代码块、引用块、`[[双链]]`、`#标签` 与锚点链接均可在往返中保留；只有 frontmatter / 上下文能确认的锚点 ID，以及唯一匹配本地文献的参考文献条目，才重建锚点与文献引用节点，未知值保留原文，不伪造锚点或 `paperId`。
- **双链按 ID 稳定解析**：`wikiLink` 节点同时保存 `noteId` 与标题快照，关系解析以 ID 优先、标题兜底；目标笔记改名后引用方无需重写正文也不会丢失链接，vault 导出仍写 Obsidian 兼容的 `[[标题]]` 并跟随新标题。
- **MCP 笔记检索提速**：`search_notes` 在 `notes_fts` 可用时改用 FTS5 trigram 查询（关键词按空白分组、AND 组合），保留 `paperId` / `pageKind` 过滤与稳定排序，短词、FTS 缺失或查询异常自动回退四字段 LIKE，旧数据库无需迁移。
- **引用样式进入设置体系**：笔记引用样式（GB/T 7714-2015 / APA 7 / IEEE）从独立 localStorage 键迁入阅读器设置，旧值一次性迁移后不再回读，异常时可保留恢复。

### 修复

- **vault 双侧修改不再静默覆盖**：同步以 frontmatter `updatedAt` 为主时钟（缺失时回退 mtime）；若文件内容与数据库在上次导出后都发生变化，原文件不再被覆盖，冲突内容写成 `--conflict-YYYYMMDD-HHmmss.md` 副本，数据库保留应用侧版本，同步结果显示冲突数量。
- **笔记双链改名不再丢关系**：目标笔记改名后，引用方再次保存不会再因旧标题匹配失败而丢失 `note_links` 关系；旧标题快照与按标题回退的解析行为保持不变。
- **摘录提炼不再污染原文快照**：公式、表格等区域的识别错误不再直接进入摘录卡——重识别失败时回退原始纯文本且不写入 `aiEnhanced`，已有锚点快照不被静默改写。
