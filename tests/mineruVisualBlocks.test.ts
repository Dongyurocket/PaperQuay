...
test("parseMineruPages unwraps page-dictionary objects generated by multi-part merge", () => {
  const pageDictionaryJson = [
    {
      "0": {
        type: "title",
        content: { title_content: [{ type: "text", content: "Architecture Optimization" }], level: 1 },
        bbox: [100, 80, 500, 120],
      },
      "1": {
        type: "paragraph",
        content: { paragraph_content: [{ type: "text", content: "This thesis presents an optimization methodology." }] },
    ...
    },
    {
      "0": {
        type: "paragraph",
        content: { paragraph_content: [{ type: "text", content: "Second page body text." }] },
        bbox: [100, 80, 500, 150],
      },
    },
  ];

  const pages = parseMineruPages(pageDictionaryJson);
  assert.equal(pages.length, 2);
  assert.equal(pages[0].length, 2);
  assert.equal(pages[1].length, 1);

  const flat = flattenMineruPages(pages);
  assert.equal(flat.length, 3);
  assert.equal(extractTextFromMineruBlock(flat[0]), "Architecture Optimization");
  assert.equal(extractTextFromMineruBlock(flat[1]), "This thesis presents an optimization methodology.");
  assert.equal(extractTextFromMineruBlock(flat[2]), "Second page body text.");
});
